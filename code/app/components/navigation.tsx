      ...
      <div className="md:hidden flex justify-center pl-3">
        {isSearchOpen ? (
          <div className="flex items-center bg-off-white rounded-lg px-2 w-full">
            <button onClick={() => { setIsSearchOpen(false); closeMenu(); }} className="ml-2 text-black hover:text-brand">
              ✖️
            </button>
            <input
              type="text"
              ...
              onKeyDown={(e) => e.key === "Enter" && handleSearch()}
            />
            <button className="ml-2 text-black hover:text-brand" onClick={() => { setIsSearchOpen(false); closeMenu(); handleSearch()}}>
          ...
          <button className="ml-2 text-black hover:text-brand" onClick={() => { setIsSearchOpen(true); closeMenu(); }}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
            </svg>
          </button>
